...
export async function run(): Promise<any> {
  ...
  // emulate tx
  try {
    console.log(`Emulating: Transfer Extra Surplus`);
    await geb.contracts.accountingJob.callStatic.workTransferExtraSurplus();

    gasUnits = await geb.contracts.accountingJob.estimateGas.workTransferExtraSurplus();
    gasUnits = gasUnits.mul(15).div(10); // add 50% buffer
    gasPrice = await provider.getGasPrice();
    txFee = gasUnits.mul(gasPrice);
    console.log(`Successfully emulated: Transfer Extra Surplus`);
  } catch (err) {
    console.log(`Unsuccessfull emulation: Transfer Extra Surplus`);
    return;
  }
  ...
  try {
    ...
    console.log(`Insufficient balance for Transfer Extra Surplus`);
  ...
  // broadcast tx
  try {
    tx = await proxy.transferExtraSurplus();
    ...
    console.log(`Successfully broadcasted: Transfer Extra Surplus`);
    return txReceipt;
  } catch (err) {
    console.log(`Failed to broadcast: Transfer Extra Surplus`);
    console.log(err);
  }
}
